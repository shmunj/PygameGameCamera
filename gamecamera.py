...
class GameCamera(object):
    ...
    def focus_on(self, target_object):
        ...
        if target_object in self.world.objects:
            if target_object.centerx - self.rect.width / 2 < 0:
                self.rect.left = 0
            elif target_object.centerx + self.rect.width / 2 > \
                                                self.world.rect.width:
                self.rect.right = self.world.rect.right
            else:
                self.rect.centerx = self.world.rect.centerx

            if target_object.centery - self.rect.height / 2 < 0:
                self.rect.top = 0
            elif target_object.centery + self.rect.height / 2 > \
                                                self.world.rect.height:
                self.rect.bottom = self.world.rect.bottom
            else:
                self.rect.centery = self.world.rect.centery
        else:
            msg = '%s is not in the %s.' % (target, self.world)
            raise(Exception(msg))
    ...
    def display(self, screen):
        ...
        screen.blit(self.surface, self.display_position)
